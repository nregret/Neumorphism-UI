...
const verticalItems: MenuItem[] = [
  { type: 'group', label: '主要功能', key: 'g1' },
  { key: 'home', label: '首页概览', icon: Home },
  { key: 'messages', label: '消息通知', icon: Bell },
  { key: 'mail', label: '收件箱', icon: Mail, disabled: true },
  { type: 'divider', key: 'd1', label: '' },
  { type: 'group', label: '系统设置', key: 'g2' },
...
<template>
  <div class="space-y-12">
    <section>
      <h1 class="text-4xl font-bold mb-4">Menu 导航菜单</h1>
      <p class="text-neu-text/80 text-lg">
        为网站提供导航功能的菜单，支持垂直和水平两种模式。
      </p>
    </section>

    <section>
      <h2 class="text-2xl font-semibold mb-6">垂直菜单 (侧边栏)</h2>
      <NeuCard class="p-12 bg-[var(--bg-color)]">
        <div class="flex gap-12">
          ...
          <div class="flex-1 p-8 rounded-2xl shadow-neu-pressed bg-[var(--bg-color)] flex items-center justify-center">
            <p class="text-neu-text/60">当前选中: <strong class="text-neu-accent">{{ activeMenu1 }}</strong></p>
          </div>
        </div>
      </NeuCard>
    </section>
    ...
    <section>
      <h2 class="text-2xl font-semibold mb-6">水平菜单 (顶栏)</h2>
      <NeuCard class="p-12 bg-[var(--bg-color)]">
        <div class="flex flex-col gap-8">
          ...
          <div class="p-8 rounded-2xl shadow-neu-pressed bg-[var(--bg-color)] flex items-center justify-center h-32">
            <p class="text-neu-text/60">当前选中: <strong class="text-neu-accent">{{ activeMenu2 }}</strong></p>
          </div>
        </div>
      </NeuCard>
