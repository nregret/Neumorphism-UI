...
const fruitOptions: SelectOption[] = [
  ...
  { label: '火龙果 (Dragonfruit)', value: 'dragonfruit' },
  { label: '接骨木果 (Elderberry)', value: 'elderberry', disabled: true },
  { label: '无花果 (Fig)', value: 'fig' },
...
<template>
  <div class="space-y-12">
    <section>
      <h1 class="text-4xl font-bold mb-4">Select 下拉选择框</h1>
    ...
    <section>
      ...
      <NeuCard class="flex flex-col gap-8 p-12 bg-[var(--bg-color)] max-w-2xl min-h-[400px]">
        <div>
          <h3 class="font-bold text-sm mb-4 text-neu-text/60">单选选择器 (Single)</h3>
          <NeuSelect 
            v-model="singleValue" 
            :options="fruitOptions" 
            placeholder="请选择你喜欢的水果" 
          />
          <p class="mt-3 text-sm text-neu-text/50">当前选择的值: <strong class="text-neu-accent">{{ singleValue || '无' }}</strong></p>
        </div>

        <hr class="border-[var(--shadow-dark)]/20 my-4" />

        <div>
          <h3 class="font-bold text-sm mb-4 text-neu-text/60">多选选择器 (Multiple)</h3>
          <NeuSelect 
            v-model="multipleValue" 
            :options="fruitOptions" 
            multiple
            placeholder="可选择多个水果" 
          />
          <p class="mt-3 text-sm text-neu-text/50">当前选择的值: <strong class="text-neu-accent">{{ multipleValue }}</strong></p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-8">
          <div>
            <h3 class="font-bold text-sm mb-4 text-neu-text/60">禁用状态 (Disabled)</h3>
            <NeuSelect 
          ...
          <div>
            <h3 class="font-bold text-sm mb-4 text-neu-text/60">错误状态 (Error)</h3>
            <NeuSelect 
              v-model="singleValue" 
              :options="fruitOptions" 
              error
