...
const stepsItems = [
  { title: '选择商品', description: '从购物车中选择' },
  { title: '填写订单', description: '确认收货地址' },
  { title: '支付款项', description: '支持多种支付方式' },
  { title: '完成交易', description: '等待商家发货' },
]

const nextStep = () => {
  if (currentStep1.value < stepsItems.length) {
    currentStep1.value++
  }
}
...
<template>
  <div class="space-y-12">
    <section>
      <h1 class="text-4xl font-bold mb-4">Steps 步骤条</h1>
      <p class="text-neu-text/80 text-lg">
        新拟态风格的步骤条，用于引导用户按照流程完成任务。
      </p>
    </section>
    ...
    <section>
      ...
      <NeuCard class="flex flex-col gap-8 p-12 bg-[var(--bg-color)]">
        ...
        <div class="flex justify-center gap-4 mt-8">
          <NeuButton @click="prevStep" :disabled="currentStep1 === 0">上一步</NeuButton>
          <NeuButton @click="nextStep" variant="primary" :disabled="currentStep1 === stepsItems.length">下一步</NeuButton>
        </div>
      </NeuCard>
    </section>
    
    <section>
      <h2 class="text-2xl font-semibold mb-6">垂直方向</h2>
    ...
    <section>
      <h2 class="text-2xl font-semibold mb-6">可点击</h2>
      <NeuCard class="p-12 bg-[var(--bg-color)]">
        <p class="mb-8 text-neu-text/60">通过设置 <code class="px-2 py-1 bg-[var(--shadow-dark)]/10 rounded">clickable</code> 属性，允许用户直接点击步骤进行跳转。</p>
