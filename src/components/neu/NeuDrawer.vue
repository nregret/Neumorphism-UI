...
const drawerClasses = computed(() => {
  return [
    'fixed bg-[var(--bg-color)] z-[110] flex flex-col shadow-neu-flat-lg',
    props.placement === 'right' ? 'top-0 right-0 h-full rounded-l-3xl' : '',
    props.placement === 'left' ? 'top-0 left-0 h-full rounded-r-3xl' : '',
    props.placement === 'bottom' ? 'bottom-0 left-0 w-full rounded-t-3xl' : '',
    props.placement === 'top' ? 'top-0 left-0 w-full rounded-b-3xl' : '',
  ].join(' ')
})
</script>
...
<template>
  <Teleport to="body">
    ...
    <Transition name="neu-drawer-fade">
      <div 
        v-if="modelValue"
        class="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm neu-drawer-overlay"
